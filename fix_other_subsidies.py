...
# IT導入補助金など他の補助金の申請方法回答を修正

import re

def update_analyze_message_function():
    """analyze_message_improved関数を更新"""
    
    try:
        with open('advisor/views.py', 'r', encoding='utf-8') as f:
            content = f.read()
        
        # 既存のanalyze_message_improved関数を置換
        new_function = '''def analyze_message_improved(message, user_context=None):
    """改良された直接メッセージ解析 - 全補助金対応版"""
...
ご不明な点があれば、お気軽にお尋ねください！""",
                'confidence_score': 0.95,
                'model_used': 'direct-it-application',
                'recommended_subsidies': []
            }
...
ご質問があれば、まずは地域の商工会議所にご相談ください！""",
                'confidence_score': 0.95,
                'model_used': 'direct-jizokuuka-application',
                'recommended_subsidies': []
            }
...
商工会議所との連携方法についてもサポートします。""",
                'confidence_score': 0.9,
                'model_used': 'direct-jizokuuka-general',
                'recommended_subsidies': []
            }
...
詳しい申請サポートについてもお尋ねください！""",
                'confidence_score': 0.95,
                'model_used': 'direct-shorikuka-application',
                'recommended_subsidies': []
            }
...
申請手順についてもお気軽にお尋ねください！""",
                'confidence_score': 0.9,
                'model_used': 'direct-shorikuka-general',
                'recommended_subsidies': []
            }
...
具体的な補助金名をお聞かせいただければ、より詳細な申請手順をご説明いたします！""",
            ...
            'model_used': 'direct-general-application',
            'recommended_subsidies': []
        }
...
より具体的なアドバイスをご提供いたします！""",
        ...
        if re.search(pattern, content, re.DOTALL):
            ...
            print("✅ analyze_message_improved関数を更新しました")
            return True
        else:
            print("❌ 関数が見つかりませんでした")
            return False
...
def main():
    ...
    if update_analyze_message_function():
        print("✅ 修正完了！")
        print("\n📋 次のステップ:")
        print("1. サーバーを再起動")
        print("2. 以下の質問でテスト:")
        print("   - 「IT導入補助金の申請方法を教えてください」")
        print("   - 「小規模事業者持続化補助金の申請方法を教えて」")
        print("   - 「省力化投資補助金の申請方法は？」")
        print("   - 「申請方法を教えてください」")
    else:
        print("❌ 修正に失敗しました")
